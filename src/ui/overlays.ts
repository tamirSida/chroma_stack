...
type ColorLinePickerArgs = {
  onConfirm: (axis: 'row' | 'col', index: number, color: 'r' | 'b' | 'g' | 'y') => void;
};

export const showColorLinePicker = (args: ColorLinePickerArgs) => {
  const overlay = ensureOverlay();
  overlay.innerHTML = '';
  ...
  wrap.className = 'color-line-picker';
  ...
  axisLabel.className = 'picker-label';
  axisLabel.textContent = 'Axis';
  wrap.appendChild(axisLabel);

  const seg = document.createElement('div');
  seg.className = 'segmented';
  const segRow = document.createElement('button');
  segRow.textContent = 'Row';
  segRow.classList.add('active');
  const segCol = document.createElement('button');
  segCol.textContent = 'Column';
  seg.appendChild(segRow);
  seg.appendChild(segCol);
  wrap.appendChild(seg);

  const idxLabel = document.createElement('div');
  idxLabel.className = 'picker-label';
  idxLabel.textContent = 'Index';
  wrap.appendChild(idxLabel);

  const idxPicker = document.createElement('div');
  idxPicker.className = 'index-picker';
  const idxBtns: HTMLButtonElement[] = [];
  for (let i = 0; i < 8; i++) {
    ...
    b.addEventListener('click', () => {
      index = i;
      idxBtns.forEach((x, j) => x.classList.toggle('active', j === i));
    });
    idxBtns.push(b);
    idxPicker.appendChild(b);
  }
  wrap.appendChild(idxPicker);
  ...
  const colorList: Array<'r' | 'b' | 'g' | 'y'> = ['r', 'b', 'g', 'y'];
  ...
  colorList.forEach((c, i) => {
    ...
    b.addEventListener('click', () => {
      color = c;
      colorBtns.forEach((x, j) => x.classList.toggle('active', j === i));
    });
    colorBtns.push(b);
  ...
  segRow.addEventListener('click', () => {
    axis = 'row';
    segRow.classList.add('active');
    segCol.classList.remove('active');
  });
  segCol.addEventListener('click', () => {
    axis = 'col';
    segCol.classList.add('active');
    segRow.classList.remove('active');
  });
  ...
  apply.className = 'btn';
  apply.textContent = 'Paint';
  apply.addEventListener('click', () => {
    hideOverlay();
    args.onConfirm(axis, index, color);
  });
